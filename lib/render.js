...
const HOME_SEO_TITLE = 'Ver Película Online en Español y Latino';
...
function head({ title, description, url, image, robots, type }) {
  // Jika ada title spesifik (halaman detail film), format menjadi: Ver [Judul Film] ([Tahun]) Película Online en Español y Latino
  // Jika di halaman utama (tanpa title), gunakan murni: Ver Película Online en Español y Latino
  let finalTitle = HOME_SEO_TITLE;
  
  if (title) {
    // Bersihkan title dari sisa nama brand jika ada, lalu format ulang sesuai permintaan murni
    const cleanTitle = title.replace(/\s*\|\s*CineBox/g, '').trim();
    finalTitle = `Ver ${cleanTitle} Película Online en Español y Latino`;
  }
    ...
    <div class="cast-grid">
      ${cast.map(person => `
        <a href="/person/${person.id}/${slugify(person.name)}" class="cast-card">
          <img src="${person.profile_path ? 'https://image.tmdb.org/t/p/w185' + person.profile_path : 'https://placeholder.com/185x278?text=No+Photo'}" alt="${escapeHtml(person.name)}" loading="lazy">
