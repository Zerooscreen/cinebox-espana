...
function layout({ headHtml, bodyHtml, activeTab }) {
  ...
  <head>
    ${headHtml}
    <!-- Tempatkan Script Adsterra / Header di sini jika diperlukan -->
  </head>
  <body>
    <header>
    ...
    <footer>
      ...
      <!-- ========================================== -->
      <!-- TEMPAT KODE HISTATS & ADSTERRA DI FOOTER   -->
      <!-- ========================================== -->
      <div style="margin-top: 15px; text-align: center;">
        <!-- Tempel kode Histats Anda di sini -->
        
        <!-- Tempel kode banner Adsterra Anda di sini -->
      </div>
    </footer>
  </body>
  </html>`;
...
function castGrid(credits) {
  ...
  return `
    <div class="cast-grid">
      ${cast.map(c => {
        const actorSlug = slugify(c.name);
        return `
          <a href="/person/${c.id}/${encodeURIComponent(actorSlug)}" class="cast-card-link">
            <div class="cast-card">
