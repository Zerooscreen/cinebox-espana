...
const DEFAULT_TITLE = 'CineBox · Películas y Series en Tendencia: Sinopsis, Valoración, Reparto y Tráiler Completo';
const DEFAULT_DESC = 'En CineBox encontrarás la sinopsis, el reparto, la valoración y el tráiler oficial de las películas y series más populares del momento, con información completa de temporadas y episodios.';
const DEFAULT_OG_IMAGE = 'https://placehold.co/1200x630/17171b/8d8a92?text=CineBox';

// TODO: inserta el código de verificación de Google Search Console de ESTE dominio
// (el código de cinebox-kr NO se puede reutilizar aquí -- está vinculado a otro dominio).
...
// ---------- Adsterra & Histats ----------
// NOTA: las keys/zonas de abajo son las MISMAS que cinebox-kr/th/de/br/fr/vn -> las
// estadísticas se mezclarán. Si quieres separarlas, crea nuevas zonas en Adsterra e Histats
// específicas para este dominio.
...
// Popunder NO se activa por defecto (riesgo de "interstitial intrusivo" y mala UX).
// Descomenta las 3 líneas de abajo + llama a popunderScript() en layout() antes de </body> si aun así quieres usarlo.
...
<header>
  <div class="header-inner">
    ...
    <nav class="tabs">
      <a class="tab-btn ${activeTab === 'movie' ? 'active' : ''}" href="/movie">Películas</a>
      <a class="tab-btn ${activeTab === 'tv' ? 'active' : ''}" href="/tv">Series</a>
    </nav>
...
<footer>
  <p>CineBox — sitio de información sobre películas y series con datos públicos de TMDB (no es un servicio de streaming) · Powered by <a href="https://www.themoviedb.org/" target="_blank" rel="noopener">TMDB</a></p>
  ${histatsSnippet()}
</footer>
...
function posterCard(item, type) {
  ...
  return `
    <a class="poster-card" href="/${type}/${item.id}/${encodeURIComponent(slug)}">
      ...
      <div class="poster-sub">${date || 'Año desconocido'}</div>
    </a>
  `;
}
...
function trailerBlock(videos) {
  ...
  if (!trailer) return `<div class="no-trailer">No hay tráiler disponible.</div>`;
  return `
...
function castGrid(credits) {
  ...
  if (!cast.length) return `<div class="empty">Información del reparto no disponible.</div>`;
